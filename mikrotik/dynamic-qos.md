...
```text
               [ Jaringan Satelit ]
                        │
                        ├─► Cuaca Cerah  --> Bandwidth Stabil (10 Mbps)
                        │                  QoS RouterOS di-set ke 9.5 Mbps
                        │
                        └─► Hujan Lebat  --> Bandwidth Turun (3 Mbps)
                                           Netwatch mendeteksi Loss / Latensi Naik
                                           Skrip berjalan: QoS RouterOS diturunkan ke 2.7 Mbps
```
...
Kita buat sebuah antrean sederhana (*Simple Queue*) sebagai induk dari semua pembagian bandwidth di LAN. Kita beri nama `sat-parent` dengan limit awal cuaca cerah sebesar **$10\text{ Mbps}$**:
...
Kita akan memantau IP gateway hub satelit (misal: `203.0.113.1`). Kita buat aturan untuk memicu skrip ketika kualitas link memburuk (latensi rata-rata $>900\text{ ms}$ atau terjadi *packet loss* $>15\%$).
...
*   `down-script`: Akan dijalankan ketika salah satu batas ambang (*threshold*) terlampaui (latensi $>900\text{ ms}$ atau loss $>15\%$).
...
Skrip ini akan menurunkan `max-limit` antrean `sat-parent` menjadi **$3\text{ Mbps}$** untuk menyesuaikan penurunan kapasitas satelit saat hujan:
...
Skrip ini akan mengembalikan `max-limit` antrean `sat-parent` ke kapasitas penuh sebesar **$10\text{ Mbps}$** setelah cuaca kembali cerah dan link stabil:
